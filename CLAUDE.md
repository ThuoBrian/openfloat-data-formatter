...
src/frontend/
  app.py           # Streamlit UI with two modes: Transform (upload → preview → validate →
                   #   download) and Statement Report (analyze OpenFloat statements)
src/tests/         # pytest suite (132 tests; statement tests use synthetic workbooks only)
```
...
- `install/`, plus `run.bat`/`start.bat`/`start.sh` at the repo root — installer/launcher scripts for non-technical users
- `.github/` — GitHub metadata, e.g. `PULL_REQUEST_TEMPLATE.md`
...
PYTHONPATH=src pytest src/tests/ -v               # Run all tests (132 tests)
...
python scripts/generate_processmaker_template.py # Regenerate docs/processmaker-input-template.xlsx
```
