...
Hard errors (invalid phone, unmapped network, invalid amount) → exclude row from output, report in validation errors. Soft warnings (duplicates, high amounts) → include row, surface in validation warnings. The validator never mutates the DataFrame.
